...
def _get_ffmpeg_path():
    ...
    if len(ffmpeg_paths) == 0:
        print("[MK-音频加载] No valid ffmpeg found.")
        return None
...
def load_audio(audio_path, start_time=0.0, duration=None, sample_rate=44100):
    ...
    try:
        ...
        print(f'[MK-音频加载] 音频加载失败: {e}')
        return None, sample_rate

    if proc.returncode != 0:
        err = proc.stderr.decode(*ENCODE_ARGS)
        print(f'[MK-音频加载] 音频加载失败 (rc={proc.returncode}): {err[:500]}')
        return None, sample_rate
...
# ═══════════════════════════════════════════════════════════════════════
_mk_decode_jobs = {}
_mk_decode_jobs_lock = threading.Lock()
_MK_DECODE_JOB_TTL = 120  # 完成后保留时间（秒）
_MK_DECODE_TIMEOUT = 600  # 解码超时（秒）
...
_mk_peaks_cache = {}
_mk_peaks_cache_lock = threading.Lock()
_MK_PEAKS_CACHE_MAX = 200  # 每条仅 500×2 个浮点数（约 8KB），200 条上限足够且内存可控


def _mk_get_cached_peaks(audio_path):
    ...
    try:
        ...
        return None
    with _mk_peaks_cache_lock:
        entry = _mk_peaks_cache.get(key)
        if not entry or entry['mtime'] != st.st_mtime or entry['size'] != st.st_size:
            _mk_peaks_cache.pop(key, None)
            return None
...
def _mk_store_peaks(audio_path, peaks, duration, sample_rate):
    """写入峰值缓存（FIFO 淘汰最旧条目）"""
    key = os.path.abspath(audio_path)
    try:
        st = os.stat(key)
        ...
        return
    with _mk_peaks_cache_lock:
        if len(_mk_peaks_cache) >= _MK_PEAKS_CACHE_MAX and key not in _mk_peaks_cache:
            oldest = min(_mk_peaks_cache.items(), key=lambda kv: kv[1]['cached_at'])[0]
            _mk_peaks_cache.pop(oldest, None)
        _mk_peaks_cache[key] = {
...
def _mk_cleanup_old_jobs():
    """清理过期的解码任务"""
    now = time.time()
    with _mk_decode_jobs_lock:
        expired = [jid for jid, job in _mk_decode_jobs.items()
                   if job.get('done') and job.get('finished_at')
                   and now - job['finished_at'] > _MK_DECODE_JOB_TTL]
        for jid in expired:
            del _mk_decode_jobs[jid]


def _mk_decode_audio_thread(audio_path, sample_rate, job_id, total_duration):
    """在后台线程中解码音频，实时更新进度。
    ...
    try:
        if not ffmpeg_path:
            with _mk_decode_jobs_lock:
                job = _mk_decode_jobs.get(job_id)
                if job:
        ...
        while True:
            ...
            if time.time() - start_time > _MK_DECODE_TIMEOUT:
                proc.kill()
                with _mk_decode_jobs_lock:
                    job = _mk_decode_jobs.get(job_id)
                    if job:
                        job['error'] = '解码超时 (%ds)' % _MK_DECODE_TIMEOUT
                        job['done'] = True
                        job['finished_at'] = time.time()
                return
        ...
        _mk_store_peaks(audio_path, peaks, actual_duration, sample_rate)
...
if getattr(PromptServer, 'instance', None) is not None:
    @PromptServer.instance.routes.get("/mk/audio_waveform")
    @mk_safe_handler
    async def get_audio_waveform(request):
        ...
        cached = _mk_get_cached_peaks(audio_path)
        ...
        if waveform is not None and waveform.shape[-1] > 0:
            _mk_store_peaks(audio_path, peaks, decoded_duration, sr)
    ...
    @PromptServer.instance.routes.post("/mk/audio_decode_start")
    @mk_safe_handler
    async def start_audio_decode(request):
        ...
        thread = threading.Thread(
            target=_mk_decode_audio_thread,
    ...
    @PromptServer.instance.routes.get("/mk/audio_decode_progress")
    @mk_safe_handler
