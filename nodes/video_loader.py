...
class MKVideoLoader:
    ...
    def load_video(self, 视频, 强制帧率=0, 视频比例="原始比例", 自定义宽度=0, 自定义高度=0,
        ...
        # 流式处理：使用生成器边解码边转换，避免一次性加载所有帧到内存
        channels = 4 if alpha else 3
        pbar = ProgressBar(yieldable if yieldable > 0 else 100)

        # 预分配tensor列表，但只存储tensor而不是numpy数组
        frame_tensors = []
        frame_count = 0

        try:
            for frame in gen:
                # 直接将numpy数组转为tensor，不保留原始numpy数组
        ...
        # 合并所有tensor
        image_tensor = torch.stack(frame_tensors).view(-1, new_h, new_w, channels)
