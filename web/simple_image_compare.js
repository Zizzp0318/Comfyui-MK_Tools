// MK_Tools - Simple Image Compare Node Frontend
...
class SimpleImageCompareWidget {
    constructor(node) {
        this.node = node;
        this.name = "mk_simple_image_compare_widget";
        this.type = "custom";
    ...
    draw(ctx, node, width, y) {
        ...
        if (this.value.images.length > 2) {
            ...
            for (const img of this.value.images) {
                const measure = ctx.measureText(img.name);
                drawData.push({ text: img.name, width: measure.width, img });
            }
...
app.registerExtension({
    name: "MK_Tools.SimpleImageCompare",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "MK_SimpleImageCompare") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                const widget = new SimpleImageCompareWidget(this);
            ...
            nodeType.prototype.onMouseEnter = function (event) {
                ...
                const widget = this.widgets?.find(w => w instanceof SimpleImageCompareWidget);
                if (widget) {
            ...
            nodeType.prototype.onSerialize = function (serialised) {
                ...
                for (let [index, widget] of (this.widgets || []).entries()) {
                    if (widget instanceof SimpleImageCompareWidget) {
